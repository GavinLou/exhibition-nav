...
function createTextTexture(
  ...
  const textHeight = Math.ceil(fontSize * 1.8);
...
class Media {
  ...
  onResize({ screen, viewport }: { screen?: ScreenSize; viewport?: Viewport } = {}) {
    ...
    // 橫向圖片：寬大於高，整體更扁，放大 50%
    const scaleFactor = 1.5;
    this.plane.scale.y = (this.viewport.height * (550 * this.scale * scaleFactor)) / this.screen.height;
    this.plane.scale.x = (this.viewport.width * (750 * this.scale * scaleFactor)) / this.screen.width;
...
class App {
  ...
  onTouchDown(e: MouseEvent | TouchEvent) {
    ...
    this.startScroll = this.scroll.current;
  }
  ...
  onClick(e: MouseEvent) {
    ...
    // 確保索引是正數
    if (actualIndex < 0) {
      actualIndex = (actualIndex + originalLength) % originalLength;
    }

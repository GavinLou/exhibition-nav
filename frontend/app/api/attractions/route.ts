...
export async function GET() {
  try {
    ...
    // 轉換數據格式以符合前端需求（保留原始欄位以支持 Step3 地圖）
    const transformedData = data.map((attraction: any) => {
      const zhTW = attraction.translations?.zh_TW || {};

      return {
        id: attraction.id,
        name: attraction.name,  // 保留原始 name
        status: attraction.status,  // 保留原始 status
        ...
        rating_count: attraction.rating_count || 0,
        estimatedDuration: 30, // 預設30分鐘（實際時長將從推薦行程取得）
        latitude: attraction.latitude,
        longitude: attraction.longitude,
        translations: attraction.translations,  // 保留完整的 translations 對象
      };
    });
